...
//    event.post       {kind: string, type: string, attributes:[{key: string, value: bytes}]}
//
...
//
//    index.scan       {start: bytes, limit: bytes, label: string, count: int}
...
func main() {
	...
	log.Printf("Service address: %v", *listenAddr)
	...
	mainHub := hub.New(mainFeed, &hub.Options{
		IndexItem: index.NewWriter(store).IndexItem,
	})
	...
	svc.subs.m = make(map[string]*hub.Listener)

	// Start up the hub.
	go mainHub.Run(ctx)
	defer func() {
		log.Printf("Closing feed: %v", mainFeed.Close())
		log.Printf("Closing hub: %v", mainHub.Wait())
	}()
	...
	if *beatInterval > 0 {
		...
		serverMetrics.SetLabel("demo.heartbeatInterval", (*beatInterval).String())
